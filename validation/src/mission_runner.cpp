...
std::expected<bool, AutopilotErrc> TrajectoryRunner::getCurrentCommand(
  ...
  auto sample =
      sampler_->getSetpoint(*trajectory_, state, sample_context, commands);
  if (!sample.has_value()) {
  ...
  switch (cfg_.exit_condition) {
    case ExitCondition::kCompleted:
      if (sample->all_finished) {
        return true;
      }
      break;

    case ExitCondition::kEquilibrium:
      if (trajectory_->checkEquilibrium(state, {}).state !=
...
std::expected<bool, AutopilotErrc> MissionRunner::getCurrentCommand(
  ...
  switch (cfg_.exit_condition) {
    ...
    case ExitCondition::kEquilibrium:
      if (active->checkEquilibrium(state, {}).state !=
          EquilibriumState::kNone) {
        return true;
      }
      break;
